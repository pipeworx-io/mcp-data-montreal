...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'DataMontreal');
}
...
async function ckanGet<T>(action: string, params: URLSearchParams): Promise<T> {
  const res = await pwFetch(`${BASE}/${action}?${params}`, { headers: { Accept: 'application/json', 'User-Agent': UA } });
  if (res.status === 429) throw new Error('upstream_throttled: donnees.montreal.ca rate limit (HTTP 429).');
  if (!res.ok) throw await httpError(res, 'donnees.montreal.ca');
