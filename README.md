...
`tools/list` at `https://gateway.pipeworx.io/data-montreal/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/montreal_recent \
  -H 'Content-Type: application/json' \
  -d '{"dataset":"crime"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/montreal_recent`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "data-montreal": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-data-montreal"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-data-montreal
```
